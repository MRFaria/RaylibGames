...
void Player::Draw(Camera2D &camera) {
    BeginMode2D(camera);
        ...
        Vector2 origin = { playerRect.width/2.0f, playerRect.height/2.0f }; // Set origin to texture's bottom right corner
...
void Player::ClampToWorldBounds() {
    ...
    if (playerRect.x + playerRect.width > GetScreenWidth() * N_TILE_SIZE) 
        playerRect.x = GetScreenWidth() * N_TILE_SIZE - playerRect.width;  // Right boundary

    if (playerRect.y < 0) playerRect.y = 0;  // Top boundary
    if (playerRect.y + playerRect.height > GetScreenHeight() * N_TILE_SIZE) 
        playerRect.y = GetScreenHeight() * N_TILE_SIZE - playerRect.height;  // Bottom boundary
}
...
bool Player::CheckCollisionsWithLevel(Level& level, Rectangle rectP, Vector2 velocityP, Vector2& contactPoint, Vector2& contactNormal, float& contactTime) {
    ...
    int playerTilePositionX = rectP.x / N_TILE_SIZE;
    int playerTilePositionY = rectP.y / N_TILE_SIZE;
    ...
    for (int x = playerTilePositionX - 10; x < playerTilePositionX + 10; x++) {
        for (int y = playerTilePositionY - 10; y < playerTilePositionY + 10; y++) {
            if (level.GetTile(x, y) == TILE_WALL) {
                Rectangle staticRect = { x * (float)N_TILE_SIZE, y * (float)N_TILE_SIZE, (float)N_TILE_SIZE, (float)N_TILE_SIZE };
